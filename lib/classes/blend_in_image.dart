...
/// ```
/// {@end-tool}
class BlendInImage extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => _BlendInImageState();
}


/// The phases a [BlendInImage] goes through.
@visibleForTesting
enum BlendInImagePhase {
  /// The initial state.
  ///
...
class _ImageProviderResolver {
  ...
  final _BlendInImageState state;
  final _ImageProviderResolverListener listener;

  BlendInImage get widget => state.widget;
...
class _BlendInImageState extends State<BlendInImage> with TickerProviderStateMixin {
  ...
  BlendInImagePhase _phase = BlendInImagePhase.start;
  BlendInImagePhase get phase => _phase;
  ...
  @override
  void didUpdateWidget(BlendInImage oldWidget) {
  ...
  void _resolveImage() {
    ...
    if (_phase == BlendInImagePhase.start)
      _updatePhase();
  }
  ...
  void _updatePhase() {
    setState(() {
      switch (_phase) {
        case BlendInImagePhase.start:
          if (_imageResolver._imageInfo != null)
            _phase = BlendInImagePhase.completed;
          else
            _phase = BlendInImagePhase.waiting;
          break;
        case BlendInImagePhase.waiting:
          if (_imageResolver._imageInfo != null) {
            ...
            _phase = BlendInImagePhase.fadeOut;
            _controller.reverse(from: 1.0);
          }
          break;
        case BlendInImagePhase.fadeOut:
          if (_controller.status == AnimationStatus.dismissed) {
            ...
            _phase = BlendInImagePhase.fadeIn;
            _placeholderResolver.stopListening();
            _controller.forward(from: 0.0);
          }
          break;
        case BlendInImagePhase.fadeIn:
          if (_controller.status == AnimationStatus.completed) {
            // Done finding in new image.
            _phase = BlendInImagePhase.completed;
          }
          break;
        case BlendInImagePhase.completed:
          // Nothing to do.
          break;
      }
    });
  ...
  Widget build(BuildContext context) {
    assert(_phase != BlendInImagePhase.start);
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder description) {
    ...
    description.add(EnumProperty<BlendInImagePhase>('phase', _phase));
